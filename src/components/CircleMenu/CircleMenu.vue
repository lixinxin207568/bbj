<template>
    ...
    <div v-for="(n,key) in menuCount" :key="key" class="sub-menu" :style="getSubMenu(n-1)">
        <router-link :to="menuUrlList[n-1].url" :target="openNewTab" :style="subMenuStyle" @mouseover.stop="mouseEnterSubMenu" @mouseout.stop="mouseOutSubMenu">
          <i :class="iconClass[n-1]" ref="icon"></i>
          <span class="">{{menuUrlList[n-1].title}}</span>
        </router-link>
      </div>
...
name:'CircleMenu',
  ...
  computed:{
    ...
    quickMenuStyle(){
      ...
      bottomPosition={bottom:'70px'},
      leftPosition = {left:'30px'},
      rightPosition = {right:'10px'}
  ...
  data(){
    return{
      menuSize:60,
      subMenu:[[["0","-100"],["-50","-80"],["-90","-45"],["-120","0"]],[["0","-160"],["80", "-138.6"],["138.6","-80"],["160","0"]],[["0","160"],["138.6","80"],["80","138.6"],["160","0"]],[["-160","0"],["-138.6","80"],["-80","138.6"],["0","160"]]],
    }
  },
  methods:{
    getSubMenu(n){
      let menuPosition = this.subMenu
